...
# from gumbel import sample_subset
...
def get_weight_assignments_pw(x, adj, pred_weight):
    ...
    x = x.permute(0, 2, 1)  # (b, n_pts, ch)
    patches = get_patches(x, adj)  # (b, n_pts, K, ch)

    patches = torch.cat([x.unsqueeze(2).repeat(1, 1, K, 1), patches], axis=-1)

    patches = patches.view(batch_size * num_points, -1, ch * 2).transpose(1,
                                                                          2)  # (b * n_pts, K, ch) -> (b * n_pts, ch, K)

    for i, module in enumerate(pred_weight):
        ...
        if i + 1 < len(pred_weight):
            patches = F.relu(patches)

    patches = patches.view(batch_size, num_points, -1, K).permute(0, 1, 3, 2)  # (b, n_pts, K, ch)

    # patches = F.softmax(patches, dim=-1)
    patches = F.softmax(patches, dim=2)

    # khot = sample_subset(patches, s=5, t=0.1, prob_dim=2, hard=True)
    # patches = patches * khot * (K / 5)
    ...
    def forward(self, x, adj, pred_weight=None):
        if self.translation_invariance == False:
            ...
            if pred_weight is None:
                ...
                q = get_weight_assignments_pw(x, adj, pred_weight)  # (b, n_pts, K, M)
            ...
            patches = torch.mul(q.unsqueeze(0), patches)
            # (b, n_pts, K, M, out_ch)
            patches = patches.permute(1, 2, 3, 4, 0)
            # add all elements for a particular m sum_{j in N_i} qwx across all neighbours -- (b, n_pts, M, out)
            patches = patches.sum(dim=2)
            # average over the number of neighbors -- (b, n_pts, M, out)
            patches = torch.mul(adj_size, patches)

            patches = patches + wx.view(batch_size, input_size, self.m_conv, self.out_channels)
...
class InnerNet(nn.Module):
    def __init__(self, in_channels, num_classes):
        ...
        out_channels_fc2 = 2048
        self.h_fc2 = nn.Sequential(CustomLinear(out_channels_fc1, out_channels_fc2), nn.ReLU())

        in_chanels_fc3 = out_channels_fc0 + out_channels_conv1 + out_channels_conv2 + out_channels_conv3 \
                         + out_channels_fc1 + out_channels_fc2 * 2
        out_channels_fc3 = 1024
        self.h_fc3 = nn.Sequential(CustomLinear(in_chanels_fc3, out_channels_fc3), nn.ReLU())

        self.y_conv = CustomLinear(out_channels_fc3, self.num_classes)

    def forward(self, x, adj, pred_weight=None):
        _, n_pts, _ = x.shape

        x0 = self.h_fc0(x)  # (b, n_pts, c)
        x1 = self.h_conv1(x0, adj, pred_weight.get('h_conv1'))
        x2 = self.h_conv2(x1, adj, pred_weight.get('h_conv2'))
        x3 = self.h_conv3(x2, adj, pred_weight.get('h_conv3'))

        x_c1 = self.h_fc1(x3)
        x_c2 = self.h_fc2(x_c1)

        pooled = torch.max(x_c2, dim=1, keepdim=True)[0]
        pooled = pooled.repeat(1, n_pts, 1)

        x = torch.cat([x0, x1, x2, x3, x_c1, x_c2, pooled], dim=-1)
        x = self.h_fc3(x)
        y = self.y_conv(x)
...
class OuterNet(nn.Module):
    def __init__(self, arch_file, in_channels=3, point_transform=True, feature_transform=True):
