            ...
            for i, input in enumerate(dataloader_test):
                ...
                if args.visualize:
                    ...
                    output_color_point_cloud(pts, label, color_map,
                                             os.path.join(args.visual_path, 'seg', obj_cat2name[objcats[category.item()]] + '_' + str(i) + '_gt.obj'))
                    output_color_point_cloud(pts, seg_pred_val, color_map,
                                             os.path.join(args.visual_path, 'seg', obj_cat2name[objcats[category.item()]] + '_' + str(i) + '_pred.obj'))
                    output_color_point_cloud_red_blue(pts, np.int32(label == seg_pred_val),
                                                      os.path.join(args.visual_path, 'seg', obj_cat2name[objcats[category.item()]] + '_' + str(i) + '_diff.obj'))
...
if __name__ == '__main__':
    ...
    # visualize setting
    if args.visualize:
        if not os.path.exists(os.path.join(args.visual_path, 'seg')):
            os.makedirs(os.path.join(args.visual_path, 'seg'))
